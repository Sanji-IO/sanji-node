...
Sanji.prototype.onMessage = function onMessage(topic, payload) {
  ...
  if (message._type === 'RESPONSE') {
    ...
    debug('get unknown type of message', message);
  }
};
  ...
  handleCallbacks = function handleCallbacks(req, res, callbacks) {
    ...
    next = function next() {
      ...
      if ('function' === typeof cb) {
        if (message._type === 'REQUEST' && callback.length > 1) {
          cb(req, res, next);
        } else if (message._type === 'EVENT' && callback.length === 1) {
          cb(req);
        } else {
          debug('message', message);
          debug('unknown handler callback', cb, cb.length);
        }
      }
    ...
    if ('function' === typeof callback) {
      if (callback.length > 1) {
        ...
        // workaround: this is for unittest spy function who's length is zero
        callback(req, res, next);
      } else {
        debug('message', message);
        debug('unknown handler callback', callback, callback.length);
      }
...
Sanji.prototype.dispatchEvent = function dispatchEvent(message) {

    var self = this,
        results, handleCallbacks;

    handleCallbacks = function handleCallbacks(req, callbacks) {
    ...
    results = self.router.dispatch(message);
    if (!results) {
      debug('Event route not found', message);
      return;
    }

    for (var rIndex = 0; rIndex < results.length; rIndex++) {
      var result = results[rIndex],
          isExit = handleCallbacks(result.message, result.callbacks);
