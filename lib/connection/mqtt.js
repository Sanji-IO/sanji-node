...
Connection.prototype.onMessage = function(topic, message) {
    ...
    try {
      ...
      log.warn('mqtt.onMessage parsed json failed: %s', message);
    }
...
Connection.prototype.connect = function() {
  var self = this;
  this._mqtt = mqtt.createClient(this.port, this.host);
...
Connection.prototype.publish = function(topic, message, options) {
  ...
  var publish = Promise.promisify(this._mqtt.publish, this._mqtt);
  options = _.merge(this.messageOptions, options);
...
Connection.prototype.setTunnel = function(newTunnel) {
  ...
  if (tunnel !== null && tunnel !== newTunnel) {
    unsubscribePromise = this.unsubscribe(tunnel)
      .tap(function() {
        log.debug('unsubscribe: %s', tunnel);
      })
      .timeout(5000)
      .catch(function(e) {
        log.debug('unsubscribe: %s failed!', tunnel);
        throw e;
      });
  }

  if (unsubscribePromise) {
    promise = unsubscribePromise.then(promise);
  }

  return promise
    .tap(function() {
      self.tunnel = newTunnel;
      log.debug('subscribe: %s', newTunnel);
    });
};
